...
class AllHpViewModel extends StateNotifier<HpState> {
  late GetHpsUsecase getHpsUsecase;
  late PostHpUsecase postHpUsecase;
  Hps? hps;
late Hps userHps = Hps(data: []);
  Hps? allHps;
  AllHpViewModel({required this.getHpsUsecase, required this.postHpUsecase})
  ...
  void isLoadedAll(Hps all) {
    state = HpState.loadedAll(hps: all);
  }
  ...
  void getHps() async {
    ...
    final either2 = await getHpsUsecase(type: "allergies");

    either2.fold((hps) {
      this.allHps = hps;

      state = HpState.loadedAll(hps: hps);
    },
        (failure) => {
